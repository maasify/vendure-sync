...
export async function vendureImport(source: string, channelCode: string) {
  ...
  async function importDefaultChannel(channel: Channel) {
    // Settings
    await importZones(); // global
    await updateChannel(channel);
    await importCountries(); // global
    await importTaxCategories(); // global
    await importTaxRates(); // global
    await importPaymentMethods(); // assign
    await importShippingMethods(); //assign
    await importRoles();
  }
  ...
  async function importZones() {
    const zones: Zone[] = readJson(TypeEnum.ZONES);

    for (const zone of zones) {
      try {
        const id = await FK.getZoneId(zone);
        ...
        console.log(`Error for zone ${zone.name} : ${e}`);
      }
    }
    FK.reset(TypeEnum.ZONES);
  }
  ...
  function readJson(type: TypeEnum) {
    try {
      return require(path.join(DIRECTORY, `${type}.json`));
    } catch (e) {
      throw `No ${type}.json file`;
    }
  }
}
